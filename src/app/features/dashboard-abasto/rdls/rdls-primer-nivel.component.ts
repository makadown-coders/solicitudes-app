...
export class RdlsPrimerNivelComponent extends AbstractTabComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        this.unidadesSrv.loadPrimerNivel().subscribe(list => {
            this.unidadesPrimerNivel.set(list ?? []);
        });

        // cuando cambie la jurisdicción (o el filtro), precargamos CPMS de esas unidades visibles
        runInInjectionContext(this.injector, (): void => {
            effect(() => {
                const visibles = this.unidadesVisibles().map(u => (u.cluesimb || '').trim().toUpperCase());
                if (!visibles.length) return;

                // 🔒 evita reentradas (teclado/scroll) y carreras
                if (this.isHydrating) return;
                this.isHydrating = true;

                (async () => {
                    try {
                        await this.precargarCpmsParaUnidades(visibles, 8);
                        this.applyCpmsDinamicos(); // ahora sí, con cache caliente
                    } finally {
    ...
    async onKitRutaChange(value: string) {
        ...
        this.balanceoService.obtenerClavesRutasSalud(kitParam).subscribe({
            next: async (resp) => {
                const set = new Set<string>((resp.claves ?? []).map((c: string) => this.norm.normClave(c)));
                this.clavesRutasSalud.set(set);

                // 1) materializa filas vacías + tipo/grupo
                this.applyTipoYGrupo();

                // 2) CPM para unidades visibles
                const visibles = this.unidadesVisibles().map(u => u.cluesimb);
                await this.precargarCpmsParaUnidades(visibles);
                this.applyCpmsDinamicos();
            },
            error: (err) => {
    ...
    exportarExcelRdlSPrimerNivel(todo = true): void {
        // en construcción
    }
    ...
    protected override async onTabActivated(): Promise<void> {
        if (this.mostradoPorPrimeraVez === false) {
            this.cargarKitsRutaSalud();
            await this.onKitRutaChange('1ER_NIVEL_147');
            // await this.onKitRutaChange('ALL');
            await this.initRows();
            this.subs.push(
                this.almSrv.existenciasAlmacenesByClave$.subscribe(map => {
                    this.applyAlmacenesBuckets(map);
    ...
    private applyCpmsDinamicos() {
        const visibles = this.unidadesVisibles()
            .map(u => (u.cluesimb || '').trim().toUpperCase())
            .filter(Boolean);
        ...
        for (const r of base) {
            ...
            for (const clues of visibles) {
                const cpm = this.cpmService.getCpmForClave(clave, clues);
                rec[clues] = cpm;
                total += cpm;
            }

            (r as any).__cpmByUnit = rec;
            (r as any).__totalCpmVisible = total;
        }
